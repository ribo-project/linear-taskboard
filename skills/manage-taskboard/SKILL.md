...
## Linear source-of-truth issues

When `issue get` returns `source: "linear"`, Linear is authoritative for issue workflow data. The local Taskboard row is a projection plus Codex execution metadata; never treat the projection as a second task system.

- When `claimEligibility` is present on a Linear issue, treat it as the server-computed autonomous **new-claim** gate. An unbound Linear issue may be selected for autonomous execution only when `claimEligibility.eligible` is exactly `true`. `claimEligibility.reasons` is diagnostic output; never override or work around a false result. Re-read the issue immediately before claiming because eligibility can change after a Linear sync.
- When a Linear `todo` already has a complete `threadBinding`, it is a continuation rather than a new claim. Use the server-computed `continuationEligibility` gate for that path: autonomous continuation is allowed only when `continuationEligibility.eligible` is exactly `true`. A false `claimEligibility` whose only reason is `ALREADY_BOUND` does not by itself reject a valid continuation; never use that exception for an unbound issue or to replace the saved binding.
- For **autonomous execution**, a Linear issue is claimable only when all of these remain true on the latest `issue get`: `status` is exactly `todo`; labels contain `codex-ready` case-insensitively; `archivedAt` is null; `linearDependencies.complete` is exactly `true`; `linearDependencies.unblocked` is exactly `true`; and `linearDependencies.unresolvedCount` is `0`. A new claim must also have no existing `threadId` or `threadBinding`.
- `linearDependencies.blockedBy` is the authoritative blocker snapshot for Linear issues and may include blockers from another Linear project or team. Do not rely only on local `relations.blockedBy`, because local relations cannot represent every cross-project Linear blocker.
- If an older automation prompt says to select or re-check Linear work using only `relations.blockedBy`, this section **supersedes that instruction for `source: "linear"` issues**. Use `claimEligibility` for an unbound new claim and `continuationEligibility` for a complete bound continuation. Keep `relations.blockedBy` as the dependency rule only for non-Linear issues.
- If `linearDependencies` is missing, `complete` is false, or the dependency snapshot is otherwise incomplete, **fail closed**: do not claim, do not continue a bound task, do not read or modify code, and do not infer that the issue is unblocked. Refresh/sync Linear and read the issue again; if it remains incomplete, stop and report.
- A `todo` with an existing complete `threadBinding` still requires `codex-ready`, a complete and unblocked dependency snapshot, and `continuationEligibility.eligible === true` before resuming. Every status write must preserve the exact saved five-field binding. Never substitute the current conversation's binding.
- Status changes and comments supported by the Linear integration write through to Linear. Unsupported Linear fields remain read-only in Taskboard; do not bypass `LINEAR_READ_ONLY`, edit the SQLite projection directly, or create a local shadow issue as a workaround.
- Before every `todo` → `in_progress` transition, re-run `issue get` after reading comments. For an unbound Linear issue re-check `claimEligibility.eligible === true`; for a complete bound continuation re-check `continuationEligibility.eligible === true` and the exact saved binding. The server also enforces this gate; `LINEAR_NOT_CLAIMABLE`, `LINEAR_BINDING_MISMATCH`, or `LINEAR_LEGACY_BINDING` means stop and reconcile instead of retrying around the guard.
...
2. Treat `backlog` as not approved for execution. Unless the user explicitly authorizes that issue, do not claim it, move it to another status, or perform task work; its assignee alone is not authorization. If work may start, claim it before reading code, downloading attachments, analyzing the implementation, or doing any other task work. Move a claimable `todo` to `in_progress` with its current `version`; do not continue until the move succeeds. For Linear issues, apply the stricter Linear source-of-truth claim or continuation conditions above. If it is already `in_progress`, continue only when it is bound to the current conversation. Never move an issue claimed by another conversation.
3. If the move conflicts because the `version` is stale, run `issue get` and `comment list` again. Retry once with the latest `version` only when the issue is still an eligible `todo`, is not bound to another conversation, is not archived, and its description and latest comments are unchanged. For an unbound Linear issue, re-check `claimEligibility.eligible === true`; for a complete bound continuation, re-check `continuationEligibility.eligible === true` and the exact five-field binding. If it was claimed, its status or requirements changed, it is archived, the service is unavailable, a permanent API error occurs, or the retry fails, stop and report. Never loop or take over another agent's claim.
