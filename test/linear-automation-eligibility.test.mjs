...
test("automation prompt uses server eligibility for Linear claims and continuations", () => {
  ...
  assert.match(prompt, /threadId、threadBinding 都為空[\s\S]*claimEligibility\.eligible/);
  ...
  assert.match(prompt, /reasons 只用于诊斷，不得绕过/);
});

test("automation prompt keeps relations.blockedBy only as the non-Linear fallback", () => {
  const prompt = buildTaskboardAutomationPrompt(request);

  assert.match(prompt, /不要使用 relations\.blockedBy 單獨判斷 Linear 是否可執行/);
  assert.match(prompt, /只有非 Linear 議題才沿用 relations\.blockedBy/);
  assert.match(prompt, /任何 todo → in_progress 狀態寫入前[\s\S]*再次執行 issue get/);
  assert.match(prompt, /未綁定新認領[\s\S]*claimEligibility\.eligible/);
