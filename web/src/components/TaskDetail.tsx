...
const ACTIVITY_FIELD_LABELS: Record<string, readonly [string, string]> = {
  ...
  developmentContext: ["開发上下文", "development context"],
  startDate: ["開始日期", "start date"],
  dueDate: ["截止日期", "due date"],
  recurrence: ["重復", "recurrence"],
  archivedAt: ["歸档狀態", "archive status"],
  relation: ["關系", "relation"],
};
...
function activityValue(
  ...
  if (field === "archivedAt") {
    return typeof value === "string"
      ? text(`已歸档（${exactTime(value, locale)}）`, `Archived (${exactTime(value, locale)})`)
      : text("未歸档", "Not archived");
  }
  ...
  return (
    <div className="issue-conversation-actions">
      <button
        ...
        type="button"
        title={text(`查看對話 ${threadId}`, `View conversation ${threadId}`)}
        onClick={onOpen}
      >
        <ConversationIcon color="currentColor" size={16} />
        <strong>{text("查看對話", "View conversation")}</strong>
      ...
      <button
        ...
        type="button"
        title={text("復制终端命令", "Copy terminal command")}
        onClick={() => onCopy(
          `codex resume ${threadId}`,
          text("Codex 恢復命令已復制。", "Codex resume command copied."),
        )}
      >
        <CodexResumeIcon />
        <span>{text("復制终端命令", "Copy terminal command")}</span>
      </button>
    </div>
  );
}
...
export function TaskDetail({
  ...
  async function saveTitle() {
    ...
    if (!normalized) {
      setTitle(currentTask.title);
      onError(["議題标题不能為空。", "Issue title cannot be empty."]);
      return;
    }
            ...
            <article className="issue-editor" aria-label={text("議題內容", "Issue content")}>
              <div className="issue-editor-content">
                <textarea
                  ...
                  rows={1}
                  value={title}
                  aria-label={text("議題标题", "Issue title")}
                ...
                {(currentTask.threadBinding || currentTask.legacyLocalThreadId) && (
                  <div
                    ...
                    aria-label={text("处理此議題的對話", "Conversations for this issue")}
                  >
                    <ConversationLink
            ...
            <section className="activity-section" aria-labelledby="activity-heading">
                      ...
                      <header className="comment-header">
                        ...
                        {comment.version > 1 && (
                          <span
                            ...
                            title={text(
                              `編輯于 ${exactTime(comment.updatedAt, locale)}`,
                          ...
                          <div className="comment-edit-actions">
                            <div className="composer-footer-leading">
                              <button
                                ...
                                type="button"
                                disabled={savingCommentId === comment.id}
                                aria-label={text("添加評論附件", "Add comment attachments")}
              ...
              <form className="comment-composer" onSubmit={(event) => { event.preventDefault(); void submitComment(); }}>
                ...
                <PendingAttachments
                  ...
                  disabled={submitting}
                  uploadLabel={text("发布後上傳", "Upload after posting")}
                ...
                <footer className="composer-footer">
                  ...
                  <div>
                    ...
                    >
                      {submitting ? text("发布中…", "Posting…") : text("評論", "Comment")}
                    </button>
                  </div>
                </footer>
              </form>
          ...
          <aside className="issue-properties" aria-label={text("議題屬性", "Issue properties")}>
            <div className="detail-primary-actions">
              ...
              >
                ...
                <span className="detail-copy-action-label">{text("復制 ID", "Copy ID")}</span>
            ...
            <div className="detail-property-row">
              <span className="detail-property-label">{text("優先級", "Priority")}</span>
              <TaskPropertyPicker
            ...
            <div className="detail-property-row development-property">
              <span className="detail-property-label">{text("開发上下文", "Development context")}</span>
              <TaskPropertyPicker
                ...
                triggerClassName="detail-property-trigger"
                ariaLabel={text("開发上下文", "Development context")}
            ...
            <div className="detail-property-row">
              <span className="detail-property-label">{text("重復", "Recurrence")}</span>
              <TaskPropertyPicker
                value={currentTask.recurrence?.unit ?? ""}
                options={[
                  { value: "", label: text("不重復", "Does not repeat"), icon: <RecurrenceIcon color="currentColor" size={14} /> },
            ...
            <div className="detail-timestamps">
              <span>{text(
                `建立于 ${exactTime(currentTask.createdAt, locale)}`,
      ...
      {pendingDelete && (
        <div className="delete-backdrop" role="presentation" onMouseDown={(event) => {
          ...
          <div className="delete-dialog" role="alertdialog" aria-modal="true" aria-labelledby="delete-comment-title">
            ...
            <p>{text("此操作無法撤销。", "This action cannot be undone.")}</p>
            <div>
      ...
      {pendingAttachmentDelete && (
        <div className="delete-backdrop" role="presentation" onMouseDown={(event) => {
          ...
          <div className="delete-dialog" role="alertdialog" aria-modal="true" aria-labelledby="delete-attachment-title">
            ...
            <p>{text(
              `“${pendingAttachmentDelete.filename}” 將被永久刪除，此操作無法撤销。`,
