...
function joinSummary(values: string[], noun: string, language: TaskboardLanguage): string | null {
  ...
  return language === "en" ? `${values.length} ${noun}` : `${values.length} 個${noun}`;
}
    ...
    return (
      <>
        ...
        <div className="task-filter-scroll" role="menu">
          ...
          {unmatched.length > 0 && (
            <>
              <div className="task-filter-section-label">{text("目前视圖中無符合條件", "No matches in this view")}</div>
              <div className="task-filter-unmatched">{unmatched.map((option) => renderOption(option, "submenu"))}</div>
            </>
          )}
          {visible.length === 0 && <div className="task-filter-no-results">{text("沒有符合條件的篩選值", "No matching filter values")}</div>}
        </div>
      </>
    );
  }
  ...
  const menu = open ? createPortal(
    ...
    >
      <label className="task-filter-search root-filter-search">
        <span className="sr-only">{text("添加篩選", "Add filter")}</span>
        <input
          ...
          placeholder={text("添加篩選…", "Add filter…")}
        />
        <kbd>F</kbd>
      </label>
      ...
      <div className="task-filter-root-list" role="menu">
        ...
        {visibleCategories.length === 0 && quickOptions.length === 0 && (
          <div className="task-filter-no-results">{text("沒有符合條件的篩選項", "No matching filters")}</div>
        )}
      </div>
