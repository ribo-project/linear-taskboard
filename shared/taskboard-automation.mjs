...
export function buildTaskboardAutomationPrompt(request) {
  ...
  const selectionInstruction = "从返回的 todo 中每次仅处理一个符合依赖条件的 todo。先按 source 分流：source=\"linear\" 且 threadId、threadBinding 都为空时，只选择 claimEligibility.eligible 严格等于 true 的新认领；source=\"linear\" 且已有完整 threadBinding 时，只选择 continuationEligibility.eligible 严格等于 true 的续跑；Linear 的对应 eligibility 字段缺失、eligible=false 或 dependency snapshot 不完整时必须 fail closed 并跳过，reasons 只用于诊断，不得绕过。不要使用 relations.blockedBy 单独判断 Linear 是否可执行。只有非 Linear 议题才沿用 relations.blockedBy 为空或其中每个依赖 status 严格等于 done 的规则。若有 todo 但没有任何符合本规则的议题，本轮直接结束，不暂停自动化，也不创建或打开新的任务会话。";
  const readInstruction = "选定候选后先用 issue get 读取最新议题内容，并用 comment list 读取全部评论。根据描述和最新评论判断是否允许开始；若其中写明等待、暂不执行或当前不应开始，立即跳过并报告，不改状态。评论也包含已完成后被打回的返工要求。";
  const recheckInstruction = "完成 issue get 和 comment list 后、任何 todo → in_progress 状态写入前，必须再次运行 issue get。若 source=\"linear\"：未绑定新认领必须再次确认 claimEligibility.eligible 严格等于 true；已有完整 threadBinding 的续跑必须再次确认 continuationEligibility.eligible 严格等于 true 且五字段 binding 与首次读取完全相同；字段缺失或 false 都立即跳过。若不是 Linear，才重新检查 relations.blockedBy。复核失败时立即结束本轮，不改状态，也不暂停自动化。";
  const executionInstructions = remoteProject
    ? [
        ...
        "确认允许开始后，只有未绑定且仍为未归档 todo 的议题才可在读取代码、下载附件、分析或实施前，由当前本地控制器使用刚读取的 version 移到 in_progress。若 source=\"linear\"，此新认领还必须使用刚才二次 issue get 返回的 claimEligibility.eligible=true；不得把 ALREADY_BOUND 或其他 reasons 当成可忽略警告。已有完整 threadBinding 时不得走新认领路径，必须先按后文旧会话 continuation 流程处理。未绑定时必须传 --clear-binding-thread，避免把本地控制器 CODEX_THREAD_ID 写成任务绑定。写入成功后记录响应 task 的 version 为 ownedVersion、projectId 为 ownedProjectId，并记录本轮 binding；以后本轮每次 issue move 都必须显式传 --if-version ownedVersion，成功后再用响应 version 更新 ownedVersion。不得省略 --if-version 后让 taskctl 自动读取最新 version。写入成功前不得继续。所有认领、评论和状态写入只由当前本地控制器完成，不得要求远程会话运行 taskctl。",
        "若因 version 陈旧发生版本冲突，重新运行 issue get 和 comment list；仅当仍为可认领 todo、绑定身份未变化、未归档且描述和最新评论未变化时，用最新 version 重试一次。Linear 未绑定议题还必须重新满足 claimEligibility.eligible=true；完整绑定续跑还必须满足 continuationEligibility.eligible=true 且 binding 不变。若已被认领、绑定、状态或要求已变、eligibility=false、已归档、服务或永久 API 错误，或重试仍失败，立即跳过该议题、退出并报告；不得抢占或循环重试。",
        "若首次 issue get 返回完整 threadBinding，议题已绑定原会话：不要走新认领。若 source=\"linear\"，必须先确认 continuationEligibility.eligible=true；否则保留原 binding 并结束本轮。通过后只能使用其保存的 threadId 和 codexHostId 调用 Codex send_message_to_thread。send 成功后必须重新 issue get 一次，确认 projectId 未变、未归档、status 仍为 todo、Linear continuationEligibility 仍为 true（若为 Linear）且完整 threadBinding 与保存值完全相同；然后由当前本地控制器使用这次复核返回的最新 version、完整旧 binding 和 --if-version 执行 issue move --status in_progress，传入 --binding-thread-id、--binding-codex-project-id、--binding-codex-project-kind、--binding-codex-host-id、--binding-workspace-path，并记录响应 task.version 为 ownedVersion。成功后继续执行后文现有 Codex wait_threads、结果评论和 in_review 写回路径，不得结束本轮；若状态写入发生 409，立即停止，不得重读新 version 覆盖。只有旧会话工具明确返回终态 NOT_FOUND 或 CLOSED 等会话不存在或已关闭结果时，才确认 stale。timeout、network failure、Codex host 暂时不可达或 Taskboard service unavailable 都不是 stale：保留 binding 并结束本轮，不得猜测、clear、create 或抢占。若任务已是 in_progress、活跃、已归档、状态或 binding 已变化，立即停止，不得在当前自动化目标创建替代会话。只有未绑定议题才使用 Codex create_thread 创建远程任务，target 必须是 {type:\"project\",projectId:actualTarget.codexProjectId,environment:{type:\"local\"}}，首次 identity 必须使用 actualTarget 的 projectId、kind=\"remote\"、hostId 和 workspacePath。发送给远程会话的指令必须包含议题编号、标题、完整描述、全部评论和开发上下文，并说明远程会话不运行 taskctl，只需完成实现、验证并返回改动、结果和剩余风险。",
        "确认旧会话 stale 后，必须先用 comment add 保存一条历史记录，并同时传 --thread-id、--binding-thread-id、--binding-codex-project-id、--binding-codex-project-kind、--binding-codex-host-id 和 --binding-workspace-path 的完整旧 binding；评论写入成功后，再使用同一次 issue get 的 version 执行 issue move --status todo --clear-binding-thread --if-version。评论或清除失败立即停止，不得认领。然后只重新 issue get 一次；仅当 projectId 未变、未归档、status 仍为 todo、threadId 为空且 threadBinding 为空，并且 Linear 议题重新满足 claimEligibility.eligible=true（若为 Linear）时，才进入未绑定议题的现有认领和 create_thread 路径。",
